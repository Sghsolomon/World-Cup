...
async function queryApi(round) {
  ...
  if (round === 'finals') {
    url =
      'https://api.football-data.org/v4/competitions/WC/matches/?stage=FINALS';

    local = '../finalsObject.json';
  } else if (round === 'semis') {
    url =
      'https://api.football-data.org/v4/competitions/WC/matches/?stage=SEMI_FINALS';
    local = '../semiFinalsObj.json';
  } else {
    url =
      'https://api.football-data.org/v4/competitions/WC/matches/?stage=QUARTER_FINALS';
